    ...
    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
        ...
        <div class="mb-4 grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    id="waktu" type="time" name="waktu" value="{{ old('waktu', $rapat->waktu->format('H:i')) }}"
                    required>
            </div>
        </div>
        ...
        <div class="mb-4">
            ...
            <select
                ...
                <option value="draft" {{ old('status', $rapat->status) == 'draft' ? 'selected' : '' }}>Draft</option>
                <option value="terjadwal" {{ old('status', $rapat->status) == 'terjadwal' ? 'selected' : '' }}>Terjadwal
                </option>
                <option value="berlangsung" {{ old('status', $rapat->status) == 'berlangsung' ? 'selected' : '' }}>
                    Berlangsung</option>
                <option value="selesai" {{ old('status', $rapat->status) == 'selesai' ? 'selected' : '' }}>Selesai
                </option>
                <option value="dibatalkan" {{ old('status', $rapat->status) == 'dibatalkan' ? 'selected' : '' }}>
                    Dibatalkan</option>
            </select>
            @error('status')
