        ...
        <table class="min-w-full table-auto">
            <thead>
                <tr class="bg-blue-200 text-gray-600 uppercase text-sm leading-normal">
                    ...
                    <th class="py-3 px-6 text-left">Pimpinan Rapat</th>
                    ...
                    <th class="py-3 px-6 text-left">Update Status Ke</th>
            ...
            <tbody class="text-gray-600 text-sm font-light">
                @forelse($rapats as $rapat)
                    <tr class="border-b border-blue-100 hover:bg-blue-100">
                        ...
                        <td class="py-3 px-6 text-left">{{ $rapat->pimpinan_rapat ?? 'Pimpinan belum ditambahkan' }}</td>
                        <td class="py-3 px-6 text-left">{{ $rapat->tanggal->format('d/m/Y') }}</td>
                        <td class="py-3 px-6 text-left">{{ $rapat->waktu }}</td>
                        <td class="py-3 px-6 text-left">{{ $rapat->tempat }}</td>
                        <td class="py-3 px-6 text-left">{{ $rapat->jenisRapat->nama }}</td>
                        <td class="py-3 px-6 text-left">
                            <span
                                class="bg-{{ $rapat->status === 'draft' ? 'yellow' : 'green' }}-200 text-{{ $rapat->status === 'draft' ? 'yellow' : 'green' }}-800 py-1 px-3 rounded-full text-xs">
                                {{ ucfirst($rapat->status) }}
                            </span>
                        </td>
                        @if (auth()->user()->role === 'admin' || $rapat->pimpinan_rapat === auth()->user()->name)
                            <td class="py-3 px-6 text-left">
                                @if($rapat->status === 'draft')
                                    <form action="{{ route('rapat.updateStatus', $rapat->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PATCH')
                        ...
                        @else
                            <td class="py-3 px-6 text-left">
                                <span class="text-red-500 text-xs">Akses anda dibatasi</span>
                            </td>
                        @endif
                        <td class="py-3 px-6 text-center">
                            <div class="flex item-center justify-center">
                                <button onclick="openViewModal('{{ $rapat->id }}')"
                                ...
                                @if (auth()->user()->role === 'admin' || $rapat->pimpinan_rapat === auth()->user()->name)
                                    <button onclick="openEditModal('{{ $rapat->id }}')"
                                        class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <button onclick="confirmDelete('{{ $rapat->id }}')"
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <h3 class="text-lg font-medium text-gray-900 mb-4">Detail Rapat</h3>
            ...
            <div class="bg-red-100 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg font-medium text-gray-900">Hapus Rapat</h3>
                        <div class="mt-2">
                            <p class="text-sm text-gray-500">
                                Apakah Anda yakin ingin menghapus rapat ini? Tindakan ini tidak dapat dibatalkan.
                            </p>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function confirmDelete(id) {
        document.getElementById('deleteForm').action = `/manajemen-rapat/${id}`;
